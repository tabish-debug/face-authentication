...
@router.post('/login')
async def login(
    response: Response, email: EmailStr = Form(), file: UploadFile = File(), 
    db: Session = Depends(get_db), Authorize: AuthJWT = Depends()):
    # Check if the user exist
    user = db.query(models.User).filter(
        models.User.email == EmailStr(email.lower())).first()
    if not user:
    ...
    # file authentication
    face_found, is_match = match_faces(user.photo.name, file)

    if not face_found or not is_match:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
                    detail='Please verify your face correctly')
